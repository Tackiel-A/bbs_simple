...
<body bgcolor="green" text="white">
    <?php
        ...
        //データベースへの接続
        $dsn = 'mysql:dbname=tb240090db;host=localhost';//データベース定義
        $user = 'tb-240090';//ユーザー名
        $pass = '9B34SdrmG8';//パスワード
            ...
            foreach($results as $row){
                if($row["id"]==$delete){
                    if($row["password"]&&$row["password"]==$password){
                        ...
                        $sql = 'UPDATE tbtest SET name=:name,comment=:comment,password=:password WHERE id=:id';
                        $stmt = $pdo->prepare($sql);
                        $stmt->bindParam(':name', $delete_mem, PDO::PARAM_STR);
                        $stmt->bindParam(':comment', $delete_mem, PDO::PARAM_STR);
    ...
    <form action="" method="post">
        ...
        <input type="password" name="password" value="<?php if($i_password){ echo $i_password;}?>" placeholder="パスワード入力"><br>
    ...
    <form action="" method="post">
        ...
        <input type="password" name="all_del_pass" placeholder="管理者用パスワード">
    </form>
